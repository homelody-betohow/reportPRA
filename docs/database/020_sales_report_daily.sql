-- 用途：销售日报表（明细表），由 Python 计算生成后写入
-- 数据样例：python/excel/base/5.1-5.5--日报.xlsx，按「日期段 + 商品ID + SKU + 站点」维度记录销售/费用/毛利
-- 字符集：utf8mb4（兼容中文与符号）
--
-- 字段对应（Excel 中文表头 -> 表字段）：
--   日期                 -> period_text / period_start / period_end
--   商品ID               -> product_id
--   SKU                  -> sku
--   站点                 -> site
--   平台（报表）         -> platform_report
--   平台                 -> platform
...
--   运营模式             -> operation_mode         （自运营/代运营）
--   供应商               -> supplier
-- 说明：原表中的「平台商品ID识别码 / 站点商品ID识别码 / 平台（报表识别码）」三列
--      均为「平台/站点 + 商品ID」拼接，属于冗余字段，已不落库；如需使用可在查询时拼接得到。

SET NAMES utf8mb4;

CREATE TABLE IF NOT EXISTS report_sales_daily (
  id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT COMMENT '自增主键',
  -- 日期段（Excel 中为 "5月1日-5月5日" 这样的字符串，落库拆为起止日期 + 原文本）
  period_month         CHAR(7)  NOT NULL COMMENT '报表月份，格式 YYYY-MM，例如 2026-04',
  period_text          VARCHAR(64) NOT NULL COMMENT '日期段原文本，例如 5月1日-5月5日',
  period_start         DATE NOT NULL COMMENT '日期段起始日（解析自 period_text）',
  period_end           DATE NOT NULL COMMENT '日期段结束日（解析自 period_text）',

  -- 商品/站点/平台 维度
  product_uid          VARCHAR(64)  NOT NULL COMMENT '商品ID，例如 25-HWLY-00002',
  product_sku          VARCHAR(64)  NOT NULL COMMENT 'SKU，例如 E56012002',
  ...
  -- 成本
  purchase_cost             DECIMAL(18,6) NOT NULL DEFAULT 0  COMMENT '采购成本',
  ...
  -- 业务自然键：同一日期段内，同一商品+SKU+站点 应只有一行
